...
/*!
 * \internal
 * \brief Apply a maintenance state change and fire the AMI notification event.
 *
...
static int apply_maintenance_state(const char *endpoint_name, int enable)
{
	...
	rc = enable ? maint_set_add(endpoint_name) : maint_set_remove(endpoint_name);
	if (rc > 0) {
 ...
 *
 * Blocks new INVITE and SUBSCRIBE dialogs (empty To-tag) with
 * 503 + Retry-After: 300.  All other requests pass through. */
...
static pj_bool_t maintenance_on_rx_request(pjsip_rx_data *rdata)
{
	...
	/* A non-empty To-tag means in-dialog; pass through. */
	...
	struct ast_cli_args *a)
{
	...
	if (rc < 0) {
		ast_cli(a->fd, "Failed to enable maintenance mode for endpoint '%s'\n",
			endpoint_name);
	} else if (rc == 0 && enable) {
		...
		ast_cli(a->fd, "Maintenance mode ENABLED for endpoint '%s'\n", endpoint_name);
	} else {
		ast_cli(a->fd, "Maintenance mode DISABLED for endpoint '%s'\n", endpoint_name);
	}
...
static int ami_set_maintenance(struct mansession *s, const struct message *m)
{
	...
	if (!strcasecmp(endpoint_name, "all")) {
		...
		it = ao2_iterator_init(all_endpoints, 0);
		while ((endpoint = ao2_iterator_next(&it))) {
			apply_maintenance_state(ast_sorcery_object_get_id(endpoint), enable);
			ao2_ref(endpoint, -1);
		}
		ao2_iterator_destroy(&it);
		ao2_ref(all_endpoints, -1);
